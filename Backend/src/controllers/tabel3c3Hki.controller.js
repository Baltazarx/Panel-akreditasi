/*
============================================================
 FILE: tabel3c3Hki.controller.js
 ...
 - Menggunakan PIVOT untuk checkmark '√' tahun perolehan.
...
// ==========================================================
const pivotHkiCheckmarkSQL = (tahunIds, aliasTabel = 'h') => `
...
================================
*/
export const listTabel3c3Hki = async (req, res) => {
  try {
    ...
    const sql = `
      ...
      FROM tabel_3c3_hki h
      
      -- JOIN untuk ambil nama dosen
      LEFT JOIN dosen d ON h.id_dosen = d.id_dosen
...
================================
*/
export const updateTabel3c3Hki = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    console.error("Error updateTabel3c3Hki:", err);
    res.status(500).json({ error: 'Gagal memperbarui data HKI', details: err.sqlMessage || err.message });
  }
};
...
export const softDeleteTabel3c3Hki = async (req, res) => {
  try {
    ...
    res.json({ message: 'Data HKI berhasil dihapus (soft delete)' });
  } catch (err) {
...
export const hardDeleteTabel3c3Hki = async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) { return res.status(404).json({ error: 'Data tidak ditemukan.' }); }
    res.json({ message: 'Data HKI berhasil dihapus secara permanen (hard delete).' });
  } catch (err) {
    console.error("Error hardDeleteTabel3c3Hki:", err);
    res.status(500).json({ error: 'Gagal menghapus data HKI secara permanen.', details: err.message });
  }
};
