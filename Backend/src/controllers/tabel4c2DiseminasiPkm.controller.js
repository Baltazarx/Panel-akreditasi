...
// ==========================================================
const pivotDiseminasiCheckmarkSQL = (tahunIds, aliasTabel = 'd') => `
    (CASE WHEN ${aliasTabel}.id_tahun_diseminasi = ${tahunIds.id_ts4} THEN '√' ELSE '' END) AS tahun_ts4,
    (CASE WHEN ${aliasTabel}.id_tahun_diseminasi = ${tahunIds.id_ts3} THEN '√' ELSE '' END) AS tahun_ts3,
    (CASE WHEN ${aliasTabel}.id_tahun_diseminasi = ${tahunIds.id_ts2} THEN '√' ELSE '' END) AS tahun_ts2,
    (CASE WHEN ${aliasTabel}.id_tahun_diseminasi = ${tahunIds.id_ts1} THEN '√' ELSE '' END) AS tahun_ts1,
    (CASE WHEN ${aliasTabel}.id_tahun_diseminasi = ${tahunIds.id_ts} THEN '√' ELSE '' END) AS tahun_ts
`;
...
export const listTabel4c2DiseminasiPkm = async (req, res) => {
  try {
    ...
    const orderBy = customOrder
      ...
      : 'pg.nama_lengkap ASC, d.id_tahun_diseminasi DESC'; // Order by nama, lalu tahun terbaru
    ...
    where.push(`d.id_tahun_diseminasi IN (?)`);
    ...
    const sql = `
      SELECT 
        ...
        ${pivotDiseminasiCheckmarkSQL(tahunIds, 'd')}
        
      FROM tabel_4c2_diseminasi_pkm d
      
      -- JOIN untuk ambil nama dosen
      LEFT JOIN dosen dsn ON d.id_dosen = dsn.id_dosen
      LEFT JOIN pegawai pg ON dsn.id_pegawai = pg.id_pegawai
      
      -- JOIN untuk ambil nama unit
      LEFT JOIN unit_kerja uk ON d.id_unit = uk.id_unit
    ...
    console.error("Error listTabel4c2DiseminasiPkm:", err);
    if (err.message.includes('ts_id tidak valid')) {
      return res.status(400).json({ error: err.message });
    }
    res.status(500).json({ error: 'Gagal mengambil daftar Diseminasi PkM', details: err.sqlMessage || err.message });
  }
};
...
================================
*/
export const getTabel4c2DiseminasiPkmById = async (req, res) => {
  try {
    const sql = `
            SELECT 
              d.*, 
              pg.nama_lengkap AS nama_dtpr,
              uk.nama_unit,
              t.tahun AS nama_tahun_diseminasi
            FROM tabel_4c2_diseminasi_pkm d
            LEFT JOIN dosen dsn ON d.id_dosen = dsn.id_dosen
            LEFT JOIN pegawai pg ON dsn.id_pegawai = pg.id_pegawai
            LEFT JOIN unit_kerja uk ON d.id_unit = uk.id_unit
            LEFT JOIN tahun_akademik t ON d.id_tahun_diseminasi = t.id_tahun
            WHERE d.id = ? AND d.deleted_at IS NULL
        `;
    const [rows] = await pool.query(sql, [req.params.id]);

    if (!rows[0]) return res.status(404).json({ error: 'Data Diseminasi PkM tidak ditemukan' });

    res.json(rows[0]);

  } catch (err) {
    console.error("Error getTabel4c2DiseminasiPkmById:", err);
    res.status(500).json({ error: 'Gagal mengambil detail data Diseminasi PkM', details: err.message });
  }
};
...
================================
*/
export const createTabel4c2DiseminasiPkm = async (req, res) => {
  try {
    const {
      id_dosen,
      judul_pkm,
    ...
    // 1. Validasi Input
    if (!id_dosen || !judul_pkm || !jenis_diseminasi || !id_tahun_diseminasi) {
      return res.status(400).json({
        error: 'Input tidak lengkap. (id_dosen, judul_pkm, jenis_diseminasi, id_tahun_diseminasi) wajib diisi.'
      });
    }

    // Validasi ENUM
    if (!['L', 'N', 'I'].includes(jenis_diseminasi)) {
      return res.status(400).json({ error: "jenis_diseminasi tidak valid. Harus 'L', 'N', atau 'I'." });
    }
    ...
    if (await hasColumn('tabel_4c2_diseminasi_pkm', 'created_by') && id_user) {
      dataToInsert.created_by = id_user;
    }

    // 5. Eksekusi Query
    const [result] = await pool.query(
      'INSERT INTO tabel_4c2_diseminasi_pkm SET ?',
      [dataToInsert]
    );
    ...
    const [rows] = await pool.query(
      `SELECT * FROM tabel_4c2_diseminasi_pkm WHERE id = ?`,
      [newId]
    );

    res.status(201).json({
      message: 'Data Diseminasi PkM berhasil dibuat',
      id: newId,
      data: rows[0]
    });

  } catch (err) {
    console.error("Error createTabel4c2DiseminasiPkm:", err);
    if (err.code === 'ER_DUP_ENTRY') {
      return res.status(409).json({ error: 'Data duplikat terdeteksi.', details: err.sqlMessage });
    }
    res.status(500).json({ error: 'Gagal membuat data Diseminasi PkM', details: err.sqlMessage || err.message });
  }
};
...
================================
*/
export const updateTabel4c2DiseminasiPkm = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    // 4. Tambah audit columns
    if (await hasColumn('tabel_4c2_diseminasi_pkm', 'updated_by') && id_user) {
      dataToUpdate.updated_by = id_user;
    }

    // 5. Eksekusi Query
    const [result] = await pool.query(
      'UPDATE tabel_4c2_diseminasi_pkm SET ? WHERE id = ?',
      [dataToUpdate, id]
    );

    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Data Diseminasi PkM tidak ditemukan atau tidak ada perubahan.' });
    }
    ...
    res.json({
      message: 'Data Diseminasi PkM berhasil diperbarui',
      data: rows[0]
    });

  } catch (err) {
    console.error("Error updateTabel4c2DiseminasiPkm:", err);
    res.status(500).json({ error: 'Gagal memperbarui data Diseminasi PkM', details: err.sqlMessage || err.message });
  }
};
...
================================
*/
export const softDeleteTabel4c2DiseminasiPkm = async (req, res) => {
  try {
    const payload = { deleted_at: new Date() };
    if (await hasColumn('tabel_4c2_diseminasi_pkm', 'deleted_by')) {
    ...
    res.json({ message: 'Data Diseminasi PkM berhasil dihapus (soft delete)' });
  } catch (err) {
    console.error("Error softDeleteTabel4c2DiseminasiPkm:", err);
...
================================
*/
export const restoreTabel4c2DiseminasiPkm = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const hasDeletedAt = await hasColumn('tabel_4c2_diseminasi_pkm', 'deleted_at');
    if (!hasDeletedAt) {
    ...
    const hasDeletedBy = await hasColumn('tabel_4c2_diseminasi_pkm', 'deleted_by');

    // Restore data
    if (hasDeletedBy) {
      const [result] = await pool.query(
        'UPDATE tabel_4c2_diseminasi_pkm SET deleted_at = NULL, deleted_by = NULL WHERE id = ? AND deleted_at IS NOT NULL',
        [id]
      );
      ...
      res.json({ ok: true, restored: true, message: 'Data Diseminasi PkM berhasil dipulihkan' });
    } else {
      const [result] = await pool.query(
        'UPDATE tabel_4c2_diseminasi_pkm SET deleted_at = NULL WHERE id = ? AND deleted_at IS NOT NULL',
        [id]
      );
    ...
    console.error("Error restoreTabel4c2DiseminasiPkm:", err);
...
================================
*/
export const hardDeleteTabel4c2DiseminasiPkm = async (req, res) => {
  try {
    const [result] = await pool.query(
      'DELETE FROM tabel_4c2_diseminasi_pkm WHERE id = ?',
      [req.params.id]
    );
    if (result.affectedRows === 0) { return res.status(404).json({ error: 'Data tidak ditemukan.' }); }
    res.json({ message: 'Data Diseminasi PkM berhasil dihapus secara permanen (hard delete).' });
  } catch (err) {
    console.error("Error hardDeleteTabel4c2DiseminasiPkm:", err);
    res.status(500).json({ error: 'Gagal menghapus data Diseminasi PkM secara permanen.', details: err.message });
  }
};
...
================================
*/
export const exportTabel4c2DiseminasiPkm = async (req, res) => {
  try {
    // Alias 'd'
    const { where, params } = await buildWhere(req, 'tabel_4c2_diseminasi_pkm', 'd');
    const orderBy = 'pg.nama_lengkap ASC, d.id_tahun_diseminasi DESC';
    ...
    const sheet = workbook.addWorksheet('Tabel 4.C.2');

    // Header Gabungan (Merge) - [5 KOLOM]
    sheet.mergeCells('E1:I1'); // -> E, F, G, H, I
    sheet.getCell('E1').value = `Tahun Diseminasi (TS = ${tahunIds.nama_ts})`;
    sheet.getCell('E1').font = { bold: true };
    sheet.getCell('E1').alignment = { horizontal: 'center' };

    // Header Kolom - [5 TAHUN]
    const headers = [
      { header: 'Nama DTPR (Ketua)', key: 'nama_dtpr', width: 30 },
      { header: 'Judul PkM', key: 'judul_pkm', width: 40 },
      { header: 'Diseminasi Hasil PkM (L/N/I)', key: 'jenis_diseminasi', width: 30 },
    ...
    sheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      if (rowNumber > 2) {
        ...
        ['jenis_diseminasi', 'tahun_ts4', 'tahun_ts3', 'tahun_ts2', 'tahun_ts1', 'tahun_ts'].forEach(key => {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=Tabel_4C2_Diseminasi_PkM.xlsx');

    await workbook.xlsx.write(res);
    res.end();

  } catch (err) {
    console.error("Error exportTabel4c2DiseminasiPkm:", err);
    if (err.message.includes('ts_id tidak valid')) {
      return res.status(400).json({ error: err.message });
    }
    res.status(500).json({ error: 'Gagal mengekspor data Diseminasi PkM', details: err.message });
  }
};