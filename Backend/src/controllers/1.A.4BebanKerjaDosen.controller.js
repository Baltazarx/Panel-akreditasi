  ...
  try {
    ...
    const sql = `
      SELECT 
        ...
        bkd.sks_pkm,
        bkd.sks_manajemen,
        (bkd.sks_pengajaran + bkd.sks_penelitian + bkd.sks_pkm + bkd.sks_manajemen) AS total_sks,
        bkd.deleted_at,
        bkd.deleted_by,
        bkd.created_at,
        bkd.updated_at
...
export const getBebanKerjaSummary = async (_req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT 
          b.id_tahun,
          AVG(b.sks_pengajaran) AS avg_pengajaran,
          AVG(b.sks_penelitian) AS avg_penelitian,
          AVG(b.sks_pkm) AS avg_pkm,
          AVG(b.sks_manajemen) AS avg_manajemen,
          (AVG(b.sks_pengajaran) + AVG(b.sks_penelitian) + AVG(b.sks_pkm) + AVG(b.sks_manajemen)) AS avg_total
       FROM beban_kerja_dosen b
...
export const createBebanKerjaDosen = async (req, res) => {
  try {
    const data = {
      ...
      sks_pengajaran: req.body.sks_pengajaran,
      sks_penelitian: req.body.sks_penelitian,
      sks_pkm: req.body.sks_pkm,
      sks_manajemen: req.body.sks_manajemen,
    };
