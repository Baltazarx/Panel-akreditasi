...
export default function C4WithTopNav() {
  ...
  const tabs = useMemo(() => {
    console.log(`[c4.jsx] Filtering tabs for role: ${role}`);
    return ALL_TABLES.filter(table => {
      const canAccess = roleCan(role, table.accessKey, "r"); // Pakai lowercase 'r'
      console.log(`[c4.jsx] - Checking access for '${table.accessKey}': ${canAccess}`);
      return canAccess;
    });
  }, [role]);
  ...
  useEffect(() => {
    ...
    const fromStore = localStorage.getItem("c4_active_tab") || "";
    ...
    console.log("C4 - Init active tab:", { fromHash, fromStore, candidate, available, chosen: next });
  ...
  useEffect(() => {
    ...
    if (url.hash !== `#tab=${activeKey}`) {
      ...
      console.log(`C4 - Updated hash to: ${url.hash}`);
    }

    localStorage.setItem("c4_active_tab", activeKey);
  }, [activeKey]);
    ...
    const checkHash = () => {
      ...
      if (fromHash && fromHash !== activeKey) {
        ...
        if (available.includes(fromHash)) {
          console.log("C4 - Hash change detected, setting active key:", { fromHash });
  ...
  if (tabs.length === 0 && role && initializedRef.current) { // Tambah initializedRef.current agar tdk flash
    return <div className="p-4 text-center text-slate-600">Tidak ada tabel C4 yang tersedia untuk peran Anda ({role}).</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="w-full rounded-2xl shadow-md px-3 sm:px-4 py-3 bg-white pt-2">
        <div className="flex items-center gap-2 mb-3">
          <h3 className="text-sm font-semibold text-slate-700">Relevansi PkM</h3>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {isLoading ? (
            <div className="text-center py-10">Memuat tabel C4...</div>
          ) : ActiveComponent ? (
