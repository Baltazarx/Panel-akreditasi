      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl md:max-w-3xl mx-4">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
            <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors">Batal</button>
            <button type="submit" className="px-5 py-2.5 rounded-lg bg-[#0384d6] hover:bg-[#043975] text-white shadow-md transition-colors">Simpan</button>
          </div>
        </form>
      </div>
    </div>
              ...
              <td className="px-6 py-4 text-center border border-slate-200">
                <div className="flex items-center justify-center gap-2">
                  {!showDeleted && canUpdate && (
                    <button onClick={() => onEdit(r)} className="font-medium text-[#0384d6] hover:underline">
                      Edit
                    </button>
                  )}
                  {!showDeleted && canDelete && (
                    <button onClick={() => onDelete(r)} className="font-medium text-red-600 hover:underline">
                      Hapus
                    </button>
                  )}
                  {showDeleted && canDelete && (
                    <button onClick={() => onHardDelete(r)} className="font-medium text-red-800 hover:underline">
                      Hapus Permanen
                    </button>
                  )}
                  {showDeleted && canUpdate && (
                    <button onClick={() => onRestore(r)} className="font-medium text-green-600 hover:underline">
                      Pulihkan
                    </button>
                  )}
                </div>
