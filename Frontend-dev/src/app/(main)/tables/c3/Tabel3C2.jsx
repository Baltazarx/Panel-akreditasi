...
const LABEL = "3.C.2 Publikasi Penelitian";
...
function ModalForm({ isOpen, onClose, onSave, initialData, maps, tahunList, authUser }) {
    ...
    if (isOpen) {
      if (initialData) {
        setForm({
          ...
          judul_publikasi: initialData.judul_publikasi || "",
          jenis_publikasi: initialData.jenis_publikasi || "",
          id_tahun_terbit: initialData.id_tahun_terbit || "",
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.id_dosen || !form.judul_publikasi || !form.jenis_publikasi || !form.id_tahun_terbit) {
      Swal.fire({
        icon: 'warning',
        title: 'Data Tidak Lengkap',
        text: 'Harap lengkapi semua field yang wajib diisi (Nama DTPR, Judul Publikasi, Jenis Publikasi, Tahun Terbit).'
      });
      return;
    }
    ...
    >
      ...
      >
        <div className="px-8 py-6 rounded-t-2xl bg-gradient-to-r from-[#043975] to-[#0384d6] text-white flex-shrink-0">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-white/80 mt-1 text-sm">Lengkapi data publikasi penelitian sesuai dengan format LKPS.</p>
            </div>
            <button
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto flex-1">
          ...
          {/* Judul Publikasi */}
          <div>
            <label htmlFor="judul_publikasi" className="block text-sm font-medium text-slate-700 mb-1">
              Judul Publikasi <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              id="judul_publikasi"
              value={form.judul_publikasi}
              onChange={(e) => handleChange("judul_publikasi", e.target.value)}
          ...
          {/* Jenis Publikasi */}
          <div>
            <label htmlFor="jenis_publikasi" className="block text-sm font-medium text-slate-700 mb-2">
              Jenis Publikasi (IB/I/S1,S2,S3,S4,T) <span className="text-red-500">*</span>
            </label>
            <div className="relative jenis-dropdown-container">
              <button
                ...
                className={`w-full px-4 py-3 border rounded-lg text-black shadow-sm focus:outline-none focus:ring-2 focus:ring-[#0384d6] focus:border-[#0384d6] flex items-center justify-between transition-all duration-200 ${form.jenis_publikasi
                  ? 'border-[#0384d6] bg-white'
                  : 'border-gray-300 bg-white hover:border-gray-400'
                  }`}
                aria-label="Pilih jenis publikasi"
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <FiShield className="text-[#0384d6] flex-shrink-0" size={18} />
                  <span className={`truncate ${form.jenis_publikasi ? 'text-gray-900' : 'text-gray-500'}`}>
                    {form.jenis_publikasi
                      ? (() => {
                        const found = jenisPublikasiOptions.find((opt) => opt.value === form.jenis_publikasi);
                        return found ? found.label : "Pilih Jenis Publikasi";
                      })()
              ...
              {openJenisDropdown && (
                ...
                >
                  {jenisPublikasiOptions.map((opt) => (
                    <button
                      key={opt.value}
                      type="button"
                      onClick={() => {
                        handleChange("jenis_publikasi", opt.value);
                        setOpenJenisDropdown(false);
                      }}
                      className={`w-full px-4 py-3 text-left flex items-center gap-3 hover:bg-[#eaf4ff] transition-colors ${form.jenis_publikasi === opt.value
                        ? 'bg-[#eaf4ff] text-[#0384d6] font-medium'
                        : 'text-gray-700'
                        }`}
                    >
          ...
          {/* Tahun Terbit */}
          <div>
            <label htmlFor="id_tahun_terbit" className="block text-sm font-medium text-slate-700 mb-2">
            ...
            <div className="relative tahun-dropdown-container">
              <button
                ...
                className={`w-full px-4 py-3 border rounded-lg text-black shadow-sm focus:outline-none focus:ring-2 focus:ring-[#0384d6] focus:border-[#0384d6] flex items-center justify-between transition-all duration-200 ${form.id_tahun_terbit
                  ? 'border-[#0384d6] bg-white'
                  : 'border-gray-300 bg-white hover:border-gray-400'
                  }`}
                aria-label="Pilih tahun terbit"
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <FiCalendar className="text-[#0384d6] flex-shrink-0" size={18} />
                  <span className={`truncate ${form.id_tahun_terbit ? 'text-gray-900' : 'text-gray-500'}`}>
                    {form.id_tahun_terbit
                      ? (() => {
                        const found = tahunOptions.find((t) => String(t.id) === String(form.id_tahun_terbit));
                        return found ? found.nama : "Pilih Tahun Terbit";
                      })()
                      : "Pilih Tahun Terbit"}
                  </span>
                </div>
                  ...
                  {tahunOptions.length === 0 ? (
                    ...
                    tahunOptions.map((t) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          handleChange("id_tahun_terbit", t.id.toString());
                          setOpenTahunDropdown(false);
                        }}
                        className={`w-full px-4 py-3 text-left flex items-center gap-3 hover:bg-[#eaf4ff] transition-colors ${form.id_tahun_terbit === t.id.toString()
                          ? 'bg-[#eaf4ff] text-[#0384d6] font-medium'
                          : 'text-gray-700'
                          }`}
                      >
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 border border-slate-200 font-semibold text-slate-800 max-w-xs">
                    <div className="truncate" title={row.judul_publikasi || ""}>
                      {row.judul_publikasi || "-"}
                    </div>
                  </td>
...
/* ---------- Page Component ---------- */
export default function Tabel3C2({ auth, role }) {
      ...
      <header className="pb-6 mb-6 border-b border-slate-200">
        ...
        <div className="flex justify-between items-center mt-1">
          <p className="text-sm text-slate-500">
            Kelola data publikasi penelitian sesuai dengan format LKPS.
          </p>
          {!loading && (
