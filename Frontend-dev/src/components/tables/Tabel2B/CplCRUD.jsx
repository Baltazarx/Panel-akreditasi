...
export default function CplCRUD({ role, maps, onDataChange, readOnly = false, refreshTrigger = 0 }) {
  ...
  const handleRestore = async (row) => {
    const result = await Swal.fire({
      title: 'Pulihkan Data?',
      text: `CPL ${row.kode_cpl} akan dikembalikan ke daftar aktif.`,
      icon: 'question',
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      try {
        await apiFetch(`/cpl/${row.id_cpl}/restore`, { method: "POST" });
  ...
  const handleHardDelete = async (row) => {
    const result = await Swal.fire({
      ...
      text: `CPL ${row.kode_cpl} akan dihapus selamanya. Tindakan ini tidak bisa dibatalkan!`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#d33',
    ...
    if (result.isConfirmed) {
      try {
        await apiFetch(`/cpl/${row.id_cpl}/hard-delete`, { method: "DELETE" });
  ...
  }, [selectedProdi, isSuperAdmin, userProdiId, refreshTrigger, showDeleted]); // Fetch ulang ketika filter berubah atau trigger refresh
          ...
          >
            ...
            {showDeleted && canUpdate && (
              <button
                ...
                aria-label={`Pulihkan data ${currentRow.kode_cpl}`}
              >
            ...
            {showDeleted && canDelete && (
              <button
                ...
                aria-label={`Hapus permanen data ${currentRow.kode_cpl}`}
              >
